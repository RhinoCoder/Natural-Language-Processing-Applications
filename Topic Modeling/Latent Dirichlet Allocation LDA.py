...
def main():
    ...
    plt.show()
    #Lets also check what we got in detail to compare it to the
    #Summary and clustering assignment we get.
    print(wrap(df.iloc[i]['text']))


    #A different text we look at.
    i = np.random.choice(len(df))
    z = Z[i]
    topics = np.arange(10) + 1
    fig, ax = plt.subplots()
